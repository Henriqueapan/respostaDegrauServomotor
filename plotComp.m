...
[erro, tempo, posicao, referencia] = textread('./Experimentos/output2024-07-22_11-47-26PDCC.txt', "%f,%f,%f,%f");
[erroA, tempoA, posicaoA, referenciaA] = textread('./Experimentos/output2024-07-22_02-23-35AvancoCCCKC.txt', "%f,%f,%f,%f");
...
tempo_maximo_de_plotA = 24;
idx_tempo_maximo_de_plotA = Inf;
...
for i=1:1:length(tempo)
    if tempoA(i) <= tempo_maximo_de_plotA + .0001 && tempoA(i) >= tempo_maximo_de_plotA - .0001
        idx_tempo_maximo_de_plotA = i
        break
    endif
endfor
...
if idx_tempo_maximo_de_plot ~= Inf
    posicaoA = posicaoA(1:idx_tempo_maximo_de_plotA)
##    vel = vel(1:idx_tempo_maximo_de_plot)
    tempoA = tempoA(1:idx_tempo_maximo_de_plotA)
endif
...
erro_filt_mediaA = filter(b, a, erroA);
...
erro_adulterado = zeros(1, length(tempo));
...
ref = ref'
erro_adulterado=erro_adulterado'

%% APENAS ERRO

figure(1)

plot(tempoA, abs(rad2deg(erro_filt_mediaA)));
hold on;
plot(tempo, abs(rad2deg(erro_filt_media)));
...
xlim([5 24])
ylim([-0.25 120])
leg = legend('Erro do Avanço de fase', 'Erro do Proporcional-Derivativo');
legend('Location','northwest');
grid on;
# Alterar conforme necessidade
title('Erro Servomotor Controlado Obtida Experimentalmente');
