...
figure(1)
##plot(tempo, abs(rad2deg(posicao_filt_media)-erro_adulterado));
plot(tempo, abs(rad2deg(posicao_filt_media)));
hold on;
...
##plot(tempo, abs(rad2deg(erro_filt_media)-erro_adulterado));
...
xlim([5 24])
ylim([-0.25 360])
leg = legend('Posição Medida', 'Posição de Referência');
legend('Location','northwest');
grid on;
# Alterar conforme necessidade
title('Posição Angular do Eixo do Servomotor Controlado Obtida Experimentalmente');
